...
- `TelemetrySender` — Timer-based sender with three-phase startup matching Pithouse's observed preamble: (0) **port probe** — sends type=0x81 session opens from port 1 upward, waits ~80ms for fc:00 acks, skips ports consumed by Pithouse/wheel; sends tier definition message via `TierDefinitionBuilder` (channel indices, compression codes, bit widths per tier as 7c:00 data chunks); probes Display sub-device via 0x43 identity commands. (1) **preamble** (~1s) — acks incoming 7c:00 channel data with fc:00, sends heartbeats only, detects Display from 0x87 response. (2) **active** — sends `0x40` channel config burst, begins `0x41` enable signal (~30 Hz), `7d:23` telemetry per tier (30ms/500ms/2000ms), `0x2D` sequence counter, mode frames, dash keepalives, display config, status pushes. Telemetry deferred until wheel is detected (`StartTelemetryIfReady`), which dispatches `Start()` to a background thread so the serial read thread stays free to deliver fc:00 ack responses during port probing. `Start()` checks `_enabled` between phases so a concurrent `Stop()` exits cleanly without creating orphaned timers. Exposes `DisplayDetected`/`DisplayModelName` for UI gating. Supports test patterns for diagnostics
- `TelemetryFrameBuilder` — Assembles complete Moza telemetry frames (`7E [N] 43 17 7D 23 ... [flag] 20 [data] [checksum]`) by bit-packing channel values from a `GameDataSnapshot`. The flag byte is the SerialStream port number discovered during probe — not user-configurable
