...
- `TelemetrySender` — Timer-based sender with a multi-phase startup matching PitHouse's observed sequence: (0) **port probe + host session opens** — sends type=0x81 session opens from port 1 upward, waits ~80ms for fc:00 acks, skips ports consumed by SimHub's built-in support or PitHouse. Gets mgmt (session 0x01) + telemetry (session 0x02) + aux (0x03) opened. (1) **device-initiated session intake** — real wheel opens sessions 0x04/0x06/0x08/0x09/0x0A on its own side ~40–400 ms after host opens 0x01/0x02; sender acks each incoming type=0x81, routes subsequent 7c:00 data to per-session handlers. (2) **dashboard upload (session 0x04)** — waits up to 2 s for device open, then if the active mzdash's MD5 doesn't match any `WheelState.EnabledDashboards[].hash` (skip-if-unchanged), sends sub-msg 1 (path registration) + sub-msg 2 (file content) + type=0x00 end marker via `FileTransferBuilder`/`DashboardUploader`, with waits for device echoes between sub-messages. (3) **configJson RPC (session 0x09)** — `ConfigJsonClient` parses the device's state blob (TitleId, enableManager dashboards, imageRefMap, etc.) as it arrives; on first complete blob the sender replies once with a `configJson()` canonical library list so the wheel's `configJsonList` picks up our profile set. (4) **tier definition + Display probe** — sends tier def on telemetry session (format depends on `ProtocolVersion`: v2 = sub-msg 1 preamble + compact numeric tier defs via `TierDefinitionBuilder`, v0 = URL subscription via `BuildV0UrlSubscription`), probes Display sub-device via 0x43 identity. (5) **preamble** (~1s) — acks incoming 7c:00 channel data with fc:00, buffers for channel catalog parsing, detects Display from 0x87 response; for v0 re-sends URL subscription (PitHouse double-sends). (6) **active** — `0x40` channel config burst (channel enables, dashboard state queries 28:00/28:01, mode 28:02), `0x41` enable signal (~30 Hz), `7d:23` telemetry per tier (30ms/500ms/2000ms), `0x2D` sequence counter, mode frames, dash/wheel keepalives (0x43 to 0x14/0x15/0x17), display config (7C:27) + dashboard activate (7C:23) interleaved per page, session acks. Two configurable settings: `ProtocolVersion` (0=URL-based, 2=compact numeric) and `FlagByteMode` (0=zero-based, 1=session-port, 2=two-batch; v2 only). Telemetry deferred until wheel is detected (`StartTelemetryIfReady`), which dispatches `Start()` to a background thread so the serial read thread stays free to deliver fc:00 ack responses during port probing. `Start()` checks `_enabled` between phases so a concurrent `Stop()` exits cleanly without creating orphaned timers. `Stop()` resets `FramesSent` + session state so disable→re-enable performs a full fresh startup. Exposes `DisplayDetected`/`DisplayModelName` for UI gating, `WheelChannelCatalog`, and `WheelState` (most-recent session 0x09 dashboard state) for UI binding. `CanonicalDashboardList` is populated by the plugin from the built-in profile set. Supports test patterns for diagnostics.
...
- `SessionDataReassembler` — Reassembles 7c:00 type=0x01 chunks. CRC-aware trailer strip (tries full-chunk CRC first, then 1-byte flag-prefix variant), `TryDecompress()` pops the 9-byte `[flag][comp_size][uncomp_size][zlib]` envelope.
