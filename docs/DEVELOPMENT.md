...
- `TelemetrySender` — Timer-based sender running at configurable intervals per tier (30ms, 500ms, 2000ms). On startup, opens a SerialStream port on the wheel via a `7c:00` type=0x81 session open frame — the port number becomes the flag byte in all telemetry frames and the session byte in status pushes (the wheel silently drops data on unopened ports). Also sends telemetry enable frames (~48 Hz), mode frames, sequence counters, heartbeats, and dash keepalives. Supports test patterns for diagnostics
- `TelemetryFrameBuilder` — Assembles complete Moza telemetry frames (`7E [N] 43 17 7D 23 ... [flag] 20 [data] [checksum]`) by bit-packing channel values from a `GameDataSnapshot`. The flag byte is the SerialStream port number opened during startup (default 0x02), not a user-configurable value
