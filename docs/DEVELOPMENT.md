...
- `MozaSerialConnection` — Device auto-discovery (WMI on Windows, probe-based under Wine/Proton), background read/write threads, frame assembly, checksum 0x7E escape (read + write). Probe sends to both base (device 19) and hub (device 18) so hub-only setups are discovered
...
- Checksum escape: when a frame's checksum byte equals `0x7E` (the start-of-frame marker), it must be doubled on the wire. `WriteLoop()` appends the extra byte after sending; `ReadLoop()` consumes it after receiving. Without this, the receiver misinterprets the checksum as a new frame start, desyncing all subsequent reads. See `docs/moza-protocol.md` § Checksum escape
