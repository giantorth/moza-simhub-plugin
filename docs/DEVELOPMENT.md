...
- `TelemetrySender` — Timer-based sender with probe-based port allocation and a two-phase startup matching Pithouse's observed preamble: (0) probes for available SerialStream ports by sending type=0x81 opens from port 1 upward, waiting ~80ms for fc:00 acks — skips ports already consumed by Pithouse or the wheel; (1) opens management + telemetry sessions on the first two available ports, subscribes to `MessageReceived` to ack incoming channel data with fc:00, sends only heartbeats for ~1 second; (2) after preamble, sends `0x40` channel config burst, then begins `0x41` enable signal (~48 Hz) and `7d:23` telemetry at configurable intervals per tier (30ms, 500ms, 2000ms). The telemetry flag byte is the probed telemetry port number. Also sends mode frames, sequence counters, dash keepalives, display config, and status pushes. Supports test patterns for diagnostics
- `TelemetryFrameBuilder` — Assembles complete Moza telemetry frames (`7E [N] 43 17 7D 23 ... [flag] 20 [data] [checksum]`) by bit-packing channel values from a `GameDataSnapshot`. The flag byte is a SerialStream port number (default 0x02) — not user-configurable. Port allocation across Pithouse sessions is an open problem (see protocol docs)
