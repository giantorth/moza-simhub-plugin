...
- `MozaSerialConnection` — Device auto-discovery (WMI on Windows, probe-based under Wine/Proton), background read/write threads, frame assembly. Probe sends to both base (device 19) and hub (device 18) so hub-only setups are discovered
