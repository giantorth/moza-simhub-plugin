...
- `TelemetrySender` — Timer-based sender with three-phase startup matching Pithouse's observed preamble: (0) **port probe + config** — sends type=0x81 session opens from port 1 upward, waits ~80ms for fc:00 acks, skips ports consumed by SimHub's built-in support or Pithouse; sends sub-message 1 session preamble (protocol version + flag base offset) then tier definition message via `TierDefinitionBuilder` (channel indices, compression codes, bit widths per tier as 7c:00 data chunks with CRC-32 on all chunks); probes Display sub-device via 0x43 identity commands. (1) **preamble** (~1s) — acks incoming 7c:00 channel data with fc:00, sends heartbeats only, detects Display from 0x87 response. (2) **active** — sends `0x40` channel config burst (channel enables, dashboard state queries 28:00/28:01, mode set 28:02), begins `0x41` enable signal (~30 Hz), `7d:23` telemetry per tier with 0-based flag bytes (30ms/500ms/2000ms), `0x2D` sequence counter, mode frames, dash/wheel keepalives (0x43 to 0x14/0x15/0x17), display config, session acks. The session port (FlagByte) is used only for 7c:00 framing — tier flag bytes in definitions and telemetry frames are always 0-based (0x00, 0x01, 0x02), independent of session port. Telemetry deferred until wheel is detected (`StartTelemetryIfReady`), which dispatches `Start()` to a background thread so the serial read thread stays free to deliver fc:00 ack responses during port probing. `Start()` checks `_enabled` between phases so a concurrent `Stop()` exits cleanly without creating orphaned timers. Exposes `DisplayDetected`/`DisplayModelName` for UI gating. Supports test patterns for diagnostics
- `TelemetryFrameBuilder` — Assembles complete Moza telemetry frames (`7E [N] 43 17 7D 23 ... [flag] 20 [data] [checksum]`) by bit-packing channel values from a `GameDataSnapshot`. The flag byte is 0-based per tier (0x00, 0x01, 0x02), independent of the session port. Note: we send 28:00/28:01 dashboard state queries during channel config but don't yet read the responses — we could use them to auto-detect which dashboard is loaded on the wheel
...
- `TierDefinitionBuilder` — Builds the tier definition message that tells the wheel firmware how to decode each flag byte's bit-packed data. Generates 7c:00 session data chunks with channel indices, firmware compression codes, and bit widths per tier. CRC-32 (ISO 3309) is appended to ALL chunks including the final one. Flag bytes are always 0-based. Compression codes confirmed from USB capture: 0x00=bool, 0x04=uint16_t, 0x07=float, 0x0D=int30, 0x0E=percent_1, 0x0F=float_6000_1, 0x14=uint3, 0x17=float_001. The `TelemetrySender` also probes the Display sub-device inside the wheel (same identity commands via group 0x43) and exposes `DisplayDetected` / `DisplayModelName` for UI gating of dashboard telemetry features
