...
| rotary-signal-mode | `2A [N]` | 1 | int | Encoder N (0–4) signal mode (0=Buttons, 1=Knob). **The answers cannot be used to count encoders** — firmware answers every index 0–4 whether or not the encoder is physically present (owner-confirmed 2026-08-25 on the KS: five answers, three knobs), so the count must be recorded per model (`WheelModelInfo.KnobEncoderCount`) and the sweep only bounds an unmeasured rim. Encoders are independent of knob LEDs — most rims have rotary encoders with no addressable knob ring, so `WheelModelInfo.KnobCount` (a group-3 LED count, non-zero only on CS Pro / KS Pro) must not be used to gate this command. PitHouse sweeps the same way, stopping at the rim's real count (`2a [N]`, N=0–3 on a 4-encoder wheel — see [`../findings/2026-04-28-wheel-catalog-read.md`](../findings/2026-04-28-wheel-catalog-read.md)). Some rims re-order: firmware index ≠ physical knob (CS Pro fw 0..3 → knobs 1,4,3,2; KS Pro fw 0..4 → knobs 1,5,4,2,3) — **note both orderings were arrived at by trial and error on the author's own rims (`4a1a8c7` → `f8d1dee` → `90c925c`, no capture cited), and `{0,4,3,1,2}` is not self-inverse, so a read/write direction mix-up would be invisible on the CS Pro but not the KS Pro.** **Storage is one bitmask, not five parameters:** the wheel commits every write to `Table 2, Param 19`, bit *N* = firmware index *N*, read-modify-write. Bundle `WDCZXZ83` (KS Pro) shows ten consecutive writes producing masks 8 → 24 → 28 → 30 → 31 → 30 → 22 → 6 → 2 → 0, each an exact single-bit set/clear, echoed on the firmware-debug channel as `param_manage.c:340 Table 2, Param 19 Written: N`; writes are acked as `bf 71 2a [N] [v]`. **Frame alias:** the read frame `40 17 2A 00 00` is byte-identical to a slot of the telemetry widget-poll cycle (`TelemetrySender.SendOneWidgetPoll`), so that poll's reply cannot be told apart from a `wheel-knob-signal-mode0` answer and lands in the same slot. **Open question — the readback disagrees with the stored mask.** In `WDCZXZ83` the wheel answered `2a 00` with `1` nine seconds before a write proved bit 0 clear, and the cold-start sweep read fw0..4 as 1,1,1,1,0 (mask 15) against a stored mask of 0. Either `2a [N]` reads report something other than the mode (`docs/moza_dissector.lua` labels `2A` "rotary-signal-mode / display-setting"), or the firmware's read-modify-write base is zeroed at boot — in which case the first per-knob write after a connect silently clears the other four. Until that is settled the plugin treats the readback as a **seed only** (`MozaData.StoreKnobSignalMode` fills a slot once and then leaves it); the per-(profile × wheel-page) overlay is the truth. To settle it, compare the `Knob signal:` block in a diagnostics bundle against the `Table 2, Param 19 Written` lines in the same bundle's Firmware debug section |
