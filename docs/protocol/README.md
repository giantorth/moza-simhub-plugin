...
>
> - **Dashboard switching** (2026-04-30/05-01): FF-record on session 0x02 is the primary switch command. Slot = 0-based index into `configJsonList` (alphabetical). Tier-def re-sent ~800ms later WITHOUT preamble (preamble only on session connect). Wheel re-pushes catalog with `\x01`-prefix URLs after switch. See [`findings/2026-04-30-dashboard-switch-3f27.md`](findings/2026-04-30-dashboard-switch-3f27.md) and [`tier-definition/handshake.md`](tier-definition/handshake.md) § In-game dashboard switch.
>
